...
Test avanzati per le challenge CTF — livelli 6-11.
...
def test_level_7_triple_encoding():
    """Level 7: three layers of encoding (hex → base64 → hex)"""
...
def test_level_11_final_boss():
    """Level 11: final boss — Morse → hex → binary → base64 → Caesar"""
...
def test_all_flags_are_unique():
    ...
    assert len(results) == len(set(results)), (
        "Some levels return the same flag — are you sure each function solves its own level?"
    )
